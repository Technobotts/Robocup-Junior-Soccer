...
public class CompassPilot extends TachoPilot
{
	...
	private float             _distance;                    // set by travel() used by regulator to
	// stop
	private byte              _direction;                   // direction of travel = sign of
	// _distance
	public float                _angle0;                      // compass heading at last call to
	 ...
	 * @param wheelDiameter Diameter of the tire, in any convenient units. (The diameter in mm is
	 *            usually printed on the tire).
	 * @param trackWidth Distance between center of right tire and center of left tire, in same
	 *            units as wheelDiameter
	 * @param leftMotor
	 * @param rightMotor
	 */
	public CompassPilot(DirectionFinder compass, float wheelDiameter, float trackWidth, TachoMotor leftMotor,
	...
	/**
	 * Returns the compass angle in degrees, Cartesian (increasing counter clockwise) i.e. the
	 ...
	 * @param distance The positive or negative distance to move the robot, same units as
	 *            _wheelDiameter
	 ...
	 * @param distance of robot movement. Unit of measure for distance must be same as wheelDiameter
	 *            and trackWidth
	 **/
	 ...
	 * @param angle degrees. Positive angle rotates to the left (clockwise); negative to the right. <br>
	...
	private void performRotation(float angle) // usd by regulator to call pilot rotate(angle, true)
	{
		...
		if(angle > 5)
			angle -= 3;
		if(angle < -5)
			angle += 3; // attempt to correct overshoot
		super.rotate(angle, false);
	}

	/**
	 * inner class to regulate rotation and travel to get direction control from compass instead of
	 * motor tacho.
	 * @author Roger Glassey
	 */
	class Regulator extends Thread
	{
		private SimplePID rotatePid = new SimplePID(0.05, 0.000001, 0);
		private SimplePID travelPid = new SimplePID(5, 0, 0);

		public void run()
		{
			travelPid.start();
			rotatePid.start();
			final int stopDist = 3;
			while(true)
			{
				while(!_regulating)
					Thread.yield();
				...
				while(_traveling)
				{
					...
					{
						float output = (float) travelPid.getOutput(getHeadingError());
						float absOut = Math.abs(output);
						if(absOut > 200)
							output = output / absOut * 200;
						steer(_direction * -output);
					}
					Thread.yield();
				}
				...
				while(_rotating)
				{
					float maxLeftSpeed = _robotTurnSpeed * _leftTurnRatio;
					float maxRightSpeed = _robotTurnSpeed * _rightTurnRatio;

					float error = getHeadingError();
					if(Math.abs(error) > stopDist)
					{
						float output = (float) rotatePid.getOutput(error);
						float speed = Math.min(Math.abs(output), 1.0f);

						_left.setSpeed(Math.round(speed * maxLeftSpeed));
						_right.setSpeed(Math.round(speed * maxRightSpeed));
